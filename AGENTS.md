...
10. Derive a profundidade de `core/INDEPENDENT_VERIFICATION.md`. Para `independent`/`adversarial`/`release`, carregue `independent-verification` e selecione apenas motores gratuitos/open source aplicáveis. Projetos simples permanecem `baseline`.
11. Use `core/EXECUTION_FABRIC.md` + `execution-router` para traduzir a ação em capacidades e eliminar backends incapazes/indisponíveis. Verificadores independentes devem preferir GitHub Actions/CI quando esse backend puder executar a prova.
12. Quando houver histórico local suficiente, aplique `core/LEARNING_ENGINE.md`/`learning-engine` somente entre candidatos já elegíveis; sem evidência suficiente, preserve o baseline.
13. Aplique `core/RISK_MODEL.md`; risco, contrato semântico, System Engineering Contract, API Engineering Contract, Independent Verification quando aplicável e Definition of Done vencem qualquer preferência aprendida.
14. Para software real novo, use `projects/<slug>/` como destino padrão e siga `core/INSPECTION_ENVIRONMENT.md` para URL canônica, preview e hospedagem.
15. Consulte `core/WORKFLOW.md` para projeto novo ou manutenção.
16. Carregue somente as Skills relevantes.
17. Consulte templates, políticas e referências apenas quando necessários.
18. Antes de criar algo do zero, verifique se existe solução consolidada, componente, biblioteca, template ou registry adequado.
19. Não misture tecnologias, bibliotecas, protocolos, scanners ou design systems sem ganho claro.
...
- ferramentas gratuitas/open source para Independent Verification por padrão;
...
Consulte o usuário quando a decisão envolver objetivo de produto, preferência subjetiva, gasto, risco destrutivo, credencial/dado indisponível ou decisão legal/organizacional. Nunca introduza ferramenta paga para Independent Verification sem autorização.
...
Novos projetos devem receber o template `templates/project/AGENTS.md`, que aponta de volta para a App Factory sem duplicar todo o Core. `templates/project/VERIFICATION.md` entra quando a profundidade de verificação justificar.
